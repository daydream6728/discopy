...
- [discopy.axioms](discopy/axioms.py) contains the property-testing module: `Axiom` states a law once on a `Testable` — `discopy.abc.Category` for the categorical laws, `axioms.Serialisable` for the roundtrips of serialisation — which also says how to generate the terms it quantifies over, and the matrix in `proptest/` checks every cell, discovering the types it quantifies over from the subclasses that implement `Testable.strategy`. Read it before adding a category or an axiom.
...
- you have `uv run` both `ruff check discopy` and `coverage run -m pytest` as described in @CONTRIBUTING.md
