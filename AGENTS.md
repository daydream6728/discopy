...
- [discopy.pattern](discopy/pattern.py) contains the sequent patterns that the `@rule`, `@generator` and `@axiom` decorators read from a signature, bounded by the abstract classes; [discopy.search](discopy/search.py) contains the rules, the generators and the search for diagrams by them. Read both before declaring structure on an abstract base class.
- [discopy.axioms](discopy/axioms.py) contains the property-testing module and documents the property-first workflow: laws stated before implementation, counterexamples recorded, strategies audited. Read it when a property test fails or before adding a category or an axiom.
