...
- [discopy.pattern](discopy/pattern.py) contains the sequent patterns a `@rule`, `@generator` or `@axiom` reads from its signature; [discopy.search](discopy/search.py) the rules, generators and the search for diagrams by them. Read both before declaring structure on an abstract base class.
