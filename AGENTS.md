...
- [discopy.search](discopy/search.py) contains the sequent patterns that the `@rule`, `@generator` and `@axiom` decorators read from a signature, and the search for diagrams by rules. Read it before declaring structure on an abstract base class.
