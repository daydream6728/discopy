...
- a generator is defined once, in the module that introduces it: `symmetric.Diagram` declares `swap_factory` with `@generator` as a method returning `Swap` and every level below gets its own `Swap` built from those of its bases, e.g. `closed.Swap = closed.Diagram.swap_factory`, and the same goes for the wires and exponentials of a `Ty`, the terms and the `Functor` of a `Diagram`; a level only writes a generator by hand when it adds behaviour to it
