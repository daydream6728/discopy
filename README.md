...
- a generator is defined once, in the module that introduces it: `symmetric` writes `@Diagram.generates` above `class Swap`, with a `ClassVar` annotation for it in the body of `Diagram`, and every level below gets its own `Swap` built from those of its bases, e.g. `closed.Swap = closed.Diagram.Swap`, and the same goes for the wires and exponentials of a `Ty`, the terms and the `Functor` of a `Diagram`; a level only writes a generator by hand when it adds behaviour to it
