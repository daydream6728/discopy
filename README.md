...
- a monoidal `Layer(*inside)` is a tensor product of boxes and plumbing, represented by non-empty `Ty`, with at least one box and no adjacent plumbing; a symmetric layer also allows `Permutation` as plumbing and may consist of a single non-identity permutation; whiskering `X @ f` and `f @ X` adds more plumbing on either side
...
- a generator is defined once, in the module that introduces it: `symmetric` declares `Diagram.swap_factory = Generator.subclass(Swap)` below the class it names, with a `ClassVar` annotation for it in the body of `Diagram`, and every level below gets its own `Swap` built from those of its bases, e.g. `closed.Swap = closed.Diagram.swap_factory`, and the same goes for the wires and exponentials of a `Ty`, the terms and the `Functor` of a `Diagram`; a level only writes a generator by hand when it adds behaviour to it
...
middle = Diagram(inside=(Layer(f, g), ), dom=A @ C, cod=B @ D)
...
CookingLayer = Recipe.layer_factory  # Every level has its own layers.
