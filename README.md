...
- a generator is defined once, in the module that introduces it: `symmetric.Diagram` declares `swap_factory` with `@generator` as a method returning `Swap` and every level below gets its own `Swap` built from those of its bases, e.g. `closed.Swap = closed.Diagram.swap_factory`; a level only writes a generator by hand when it adds behaviour to it
