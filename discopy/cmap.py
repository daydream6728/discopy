...
class CMap[C0: Pregroup, C1: CMap](
    ...
    def node(self) -> Permutation:
        """ Compatibility alias for the box-only orientation permutation. """
        return Permutation.from_cycles(
            self._box_port_indices, len(self.ports))

    @property
    def edge(self) -> Permutation:
        """ Compatibility alias for the edge involution. """
        return self.edges
    ...
    def assert_rooted_map(self):
        ...
        if self.node[-1] != self.n_ports - 1:
            raise ValueError
        if self.edges[-1] == self.n_ports - 1:
            raise ValueError
