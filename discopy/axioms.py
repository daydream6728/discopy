...
<https://hypothesis.readthedocs.io>`_: an :class:`discopy.search.Axiom`
is stated once on an abstract base class as a sequent, a category
generates its own objects and arrows through :class:`Strategy` — by the
rules of :mod:`discopy.search` for diagrams — and the matrix in
``proptest/`` searches every cell for a counterexample.
...
- ``proptest/test_axioms.py`` is the matrix: every
  :class:`discopy.search.Axiom` of every category in ``CATEGORIES``, one
  pytest cell per pair, arguments generated by
  :meth:`discopy.search.Axiom.strategy` from the sequent the law's own
  signature states.
...
1. **State the laws.** Which equations define the new structure? Which
   ...
   must the new category inherit, compare
   :meth:`discopy.search.Axiom.modulo` a quotient, declare
   :meth:`discopy.search.Axiom.inapplicable` — or
   :meth:`discopy.search.Axiom.weaken` to the subspace a strategy
   parameter cuts out, so that a :meth:`discopy.search.Axiom.failing` law
   with a green subspace shows one expected failure and one green cell?
   Write this down before any implementation, as a sequent: the variables
   and their sorts, the premises and their types.
2. **Scaffold the axioms.** Declare each law as an
   :func:`discopy.search.axiom` on the abstract base class — or an ad-hoc
   property in its ``proptest/`` file when it is a boolean rather than an
   equation — and enrol the category in ``CATEGORIES``. The
...
3. **Debug against the record**, not the search. In a REPL, call the
   ...
   :meth:`discopy.search.Axiom.falsify` to reproduce a known failure: it
   searches and shrinks afresh each run and may land on a different
...
5. **Or file it.** If the fix is out of scope, open an issue, declare the
   ...
   the :meth:`discopy.search.Axiom.failing` declaration is removed — at
   which point the record is the pin. The search cell xfails too, without
   strictness: whether a search finds a rare counterexample within its
   budget is not a fact about the law.

A counterexample against an ad-hoc property that has no
:class:`discopy.search.Axiom` follows the same steps, except the record is
a plain regression test in the module's ``test/`` file.
...
- ``axiom`` is the class attribute access, which binds the
  :class:`discopy.search.Axiom` to its category — the same object the
  matrix checks, so a record can never drift from the law it witnesses.
...
declared :meth:`discopy.search.Axiom.failing`, and checks the equation the
axiom's :class:`discopy.search.AxiomFailure` carries, so the xfail is
earned by the arguments falsifying the law in one of the two shapes
:meth:`discopy.search.Axiom.falsify` counts:
...
3. **Observation.** Drawn but not seen: the law compares its equation
   :meth:`discopy.search.Axiom.modulo` a quotient that erases the
   difference, states
...
class Equation(NamedGeneric["ar"]):
    ...
    def modulo(self, up_to: Callable) -> Self:
        """
    ...
    for category in categories:
        for law in category.axioms.values():
            ...
            try:
                ...
                assert verdict is NotImplemented or verdict, law
