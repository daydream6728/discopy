...
<https://hypothesis.readthedocs.io>`_: an :class:`Axiom`
...
- ``proptest/test_axioms.py`` is the matrix: every :class:`Axiom` of
  every category in ``CATEGORIES``, one pytest cell per pair, its
  equations generated by :meth:`Axiom.strategy`: the arguments drawn
  from the patterns of the sequent the law's own signature states, and
  the law evaluated on them.
...
1. **State the laws.** Which equations define the new structure? Which
   ...
   :meth:`Axiom.modulo` a quotient, declare
   :meth:`Axiom.inapplicable` — or
   :meth:`Axiom.weaken` to the subspace a strategy
   parameter cuts out, so that a :meth:`Axiom.failing` law
...
3. **Debug against the record**, not the search. In a REPL, call the
   ...
   :meth:`Axiom.falsify` to reproduce a known failure: it
...
5. **Or file it.** If the fix is out of scope, open an issue, declare the
   ...
   the :meth:`Axiom.failing` declaration is removed — at
...
:class:`Axiom` follows the same steps, except the record is
...
- ``axiom`` is the class attribute access, which binds the
  :class:`Axiom` to its category — the same object the
...
declared :meth:`Axiom.failing`, and checks the equation the
axiom's :class:`AxiomFailure` carries, so the xfail is
earned by the arguments falsifying the law in one of the two shapes
:meth:`Axiom.falsify` counts:
...
3. **Observation.** Drawn but not seen: the law compares its equation
   :meth:`Axiom.modulo` a quotient that erases the
   difference, states
