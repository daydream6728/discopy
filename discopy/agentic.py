...
class Diagram:
    ...
    def downgrade(self) -> category:
        ...
        This is the identity functor read as landing in the underlying
        category, which is what sends the plumbing of :func:`lift_structure`
        back to the swaps and copies it was lifted from.
        ...
        return self.functor_factory(
            ob_map=lambda x: x, ar_map=lambda box: box,
...
class Agentic:
    ...
    def __class_getitem__(cls, values) -> type:
        ...
        if (category, box) not in cls.cache:
            ...
            lift_structure(diagram, category, name)
            cls.cache[category, box] = diagram
        return cls.cache[category, box]


def lift_structure(diagram: type, category: type, name: str = "") -> None:
    """
    Point the structural factories of `diagram` at subclasses of both the box
    they generate and `diagram` itself, i.e. lift the plumbing of the
    underlying `category` into the free category on the prompts.

    A swap or a copy generated by `category` is a diagram of `category` and
    not of `diagram`, so without this it refuses to compose with a prompt and
    :meth:`Diagram.refine` raises ``TypeError`` on any plan with plumbing in
    it. The factories are found by name, i.e. every ``*_factory`` of the
    underlying category which generates one of its diagrams.

    Parameters:
        diagram : The `Agentic` subclass whose factories to repoint.
        category : The underlying category.
        name : The suffix for the names of the subclasses.

    Example
    -------
    >>> from discopy.markov import Copy, Diagram, Ty
    >>> D = Agentic[Diagram]
    >>> assert issubclass(D.copy_factory, Copy)
    >>> assert issubclass(D.copy_factory, D)
    >>> assert D.braid_factory is D.swap_factory
    >>> x = Ty('x')
