...
- [x] A `Ledger` in `discopy.testing`: the pass/fail history of each
...
- [x] A `proptest/conftest.py` that swaps each cell's budget in before it
  runs and records its outcome after, fully automatic, no test file
  edited.
- [x] The `proptest` workflow restores the latest ledger artifact before
  the run and uploads the updated ledger after.
- [x] PROPTEST.md and CHANGELOG.md describe the adaptive budget.
