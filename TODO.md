...
- [x] `testing.Axiom.broken` scans `co_names` for the literal `"AxiomError"` —
  valid: `.failing` is now the only producer of brokenness, so set an explicit
  flag there and propagate it through `bind`/`modulo`/`weaken`.
- [x] `cat.Functor.strategy` declares `dom`/`cod` and ignores them — valid,
  and the pasting-diagram generators do pass them: honour the boundaries by
  filtering instead of dropping the parameters.
- [x] `abc.Category.equation_factory` docstring names both alternatives
  `cls.equation_factory` — valid, restate the strict-vs-quotient choice.
- [x] CHANGELOG "Added" describes the removed strict/setoid status system as
  shipped — valid, fold into the entry that removes it.
- [x] `Matrix.strategy` ignores `dtype` — deliberate: integer entries are
  exact in every enrolled dtype where floats would make strict equations
  flaky; say so in the docstring.
- [x] broken axioms can XPASS when 25 draws miss the counterexample — the
