...
class Circuit(monoidal.Diagram):
    ...
    def __call__(self, *bits):
        F = monoidal.Functor(
            ob_map=lambda _: bool, ar_map=lambda f: f.data,
            cod=python.Function)
        return F(self)(*bits)
...
Equation(spiral(8), spiral(8).dagger())
```
...
F = traced.Functor(ob_map={x_2: float}, ar_map={div: lambda x, y=1.0: x / y, add: lambda x, y: x + y, copy: lambda x: (x, x), one: lambda: 1.0}, cod=python.Function)
...
Cup, Cap, Variable = Kauffman.Cup, Kauffman.Cap, Kauffman.Box
...
assert symmetric.Equation(diagram_left, diagram_right)
symmetric.Equation(diagram_left, diagram_right)
```
...
frobenius.Equation(diagram_lhs, diagram_rhs)
```
...
class Formula(frobenius.Diagram):
    ...
    def eval(self, size, model):
        return frobenius.Functor(
            ob_map=lambda _: Dim(size), ar_map=lambda f: model[f],
