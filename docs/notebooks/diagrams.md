...
This is implemented by the class `Functor`, which is initialised by a pair of maps `ob_map` from objects to objects and `ar_map` from boxes to arrows.
Functors also have an optional `cod: Category`, i.e. a pair of classes for objects and arrows, `Ob` and `Arrow` by default.
Note that the domain of a DisCoPy functor is implicit: it is the free category generated by the domain of its `ob_map` and `ar_map` maps.
<!---->
...
F = Functor(
    ob_map={white: blanc, foam: mousse, meringue: meringue},
    ar_map={whisk: saler >> battre, bake: cuire >> refroidir},
...
**Example:** We can define a functor $F : \mathbf{C}_G \to \mathbf{Pyth}$ which sends cooking recipes to string-valued functions. In this case, it is easier to use functions rather than dictionaries for the `ob_map` and `ar_map` mappings.
...
G = Functor(
    ob_map=lambda _: (str, ),
    ar_map=lambda box: lambda x: box.name.removesuffix('e') + f'ed {x}',
...
H = Functor(
    ob_map={white: Cooking, foam: Cooking, meringue: Pyth},
    ar_map={whisk: F, bake: G},
    dom=Cooking, cod=Cat)
...
class Valuation(Functor):
    ...
    def __init__(self, ar: dict[Atom, bool]):
        super().__init__(ob_map={x: 1}, ar_map=ar, cod=Mat_B)
...
assert f_1 @ g_1 @ h_1 == f_1 @ (g_1 @ h_1)
# Associativity
Equation(f_1 >> g_1 >> h_1, f_1 @ g_1 @ h_1, symbol='and')
```
...
Equation(left_interchange, Equation(effect >> state, right_interchange, symbol='$\\longrightarrow$'), symbol='$\\longleftarrow$')
```
...
Equation(u @ v, v @ u, u @ v, symbol="$\\longrightarrow$")
```
...
mo.vstack([
    Equation(spiral(i), spiral(i).normal_form())
    for i in range(1, 4)])
```
