...
- `discopy.owl`, the category of relations of an OWL ontology, split at
  ...
  the reasoner that judges it — HermiT by default, through the `owlapi`
  bridge whose jars `owlapy` bundles — `load` pulls an ontology in from
  its base URL, or offline from a directory scanned by `owlapi`'s own
  IRI mapper, and everything semantic is decided by the reasoner —
  nothing is concluded from absence. `Relation` is the underlying
  single-sorted category generated by `owl:Thing`: objects are arities,
  morphisms are finite relations between tuples of individuals — the
  entailed atoms, read off the reasoner directly — with converse, meet,
  join and bottom but no complement or top, since OWL cannot say either
  of a property; a predicate lives there as the coreflexive that
  `extension` returns.
  `Query` is its Karoubi envelope restricted to the coreflexives: the
  ...
  the predicate changes; `no_reasoning` postpones the proofs and
  `validate` settles them, raising on every hand-off HermiT refutes.
  Retrieval and proof are direct reasoner calls, with no scratch
  classes and no writeback: `instances` retrieves the entailed members
  of a class expression, memoised on the world until an axiom is added,
  `subsumes` asks `is_entailed` on the subclass axiom itself,
  `consistent` checks worlds, and `Relation.sparql` queries the
  asserted graph with `rdflib`. `axioms` compiles what the ontology
  says into `Axiom`, whose truth is the absence of an entailed
  counterexample — a consistent schema entails itself, and a `False`
  axiom exhibits a concrete entailed witness — one `owlapy` axiom at a
  time through `class_axioms` and `property_axioms`, or a whole `World`
  ...
  `owlapy` entity it denotes as `data`. Each relation carries an
  ...
  `Equation` — intersection is composition, a quantifier follows its
  property and discards, the complement is the one bubble of the
  dictionary with a union drawn as its De Morgan dual (`demorgan`) and
  a cardinality's inequality as complemented cups (`distinct`, the cup
  being the equality test of a frobenius category), and a class axiom
  is read at its subject's own predicate, a typed wire with the
...
- Abstract base classes for order-enriched categories in `discopy.abc`:
  ...
  dagger category enriched in posets with local meets,
  `DistributiveAllegory` for lattice hom-sets with a `bottom` whose joins
  composition preserves, and `BooleanAllegory` for hom-sets that are
  Boolean algebras with a `top`, whose instances are quantaloids. The
  category of relations of an OWL ontology, `discopy.owl`, implements
  the distributive one — OWL has no complement of a property.
