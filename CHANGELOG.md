...
- `discopy.owl`, the category of relations of an OWL ontology, split at
  its predicates and deductive throughout: `load` pulls an ontology in
  from its base URL with `owlready2` (or offline from a directory with
  `preload`), and everything semantic is decided by HermiT — nothing is
  concluded from absence. `Relation` is the underlying single-sorted
  category generated by `owl:Thing`: objects are arities, morphisms are
  finite relations between tuples of individuals — the entailed atoms
  once `reason` has run — with converse, meet, join and bottom but no
  complement or top, since OWL cannot say either of a property; a
  predicate lives there as the coreflexive that `extension` returns.
